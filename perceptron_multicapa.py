...
class Perceptron_multicapa:
    ...
    def activación(self):
        ...
        #después aplicación de la función sigmoide (tf.sigmoid)
        activacion_capa_oculta = tf.sigmoid(tf.matmul(activacion, self.peso_capa_oculta) + self.sesgo_capa_oculta)

        #Función de error de media cuadrática MSE
        self.funcion_error = tf.reduce_sum(tf.pow(self.tf_valores_reales_Y-activacion_capa_oculta,2))
    ...
    #Visualización gráfica
        ...
        for i in range(0,4):
            print("Observación:"+str(self.valores_entradas_X[i])+ " - Esperado: "+str(valores_a_predecir_Y[i])+" - Predicción: "+str(self.sesion.run(activacion_capa_oculta, feed_dict={tf_neuronas_entradas_X: [valores_entradas_X[i]]})))
